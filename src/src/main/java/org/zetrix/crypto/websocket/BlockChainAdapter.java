/*
Copyright Zetrix Technologies Co., Ltd. 2017 All Rights Reserved.
...
package org.zetrix.crypto.websocket;
...
import org.zetrix.crypto.protobuf.Common.*;
	...
	private class BlockChainManager implements Runnable {
		...
		private class BlockChain extends WebSocketClient {
			...
			public void onClose( int code, String reason, boolean remote ) {
				...
				logger_.warn( "Closed: " + index_ + ", code:" + code + ", reason:" + reason );
			}
			...
			public void onMessage(String message) {
				try {
					...
					if (request && request_handler_[type] != null) {
						...
						logger_.warn( "onMessage:" + (request ? " request method" : " reponse method") + " (" + type + ")" + " does not exist");
					}
				} catch (Exception e) {
			...
			public void onMessage(ByteBuffer message) {
				try {
					...
					int type = (int)wsMessage.getType();
					boolean request = wsMessage.getRequest();
					//System.out.println("message type: " + type + ", requestType: " + request);
					if (request && request_handler_[type] != null) {
						byte[] msg = wsMessage.getData().toByteArray();
						request_handler_[type].ChainMethod(msg, msg.length);
					}
